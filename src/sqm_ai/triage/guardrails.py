...
DIMENSIONAL = re.compile(
    ...
    r"|diameter|position|true position)\b",
    I,
)
MARKINGS = re.compile(r"\b(ITAR|CUI|export controlled)\b", I)
PII_PATTERNS = [
...
def contains_pii(text: str) -> bool:
    """Flag selected contact/identifier patterns; not a complete PII detector."""
...
def validate(
    ...
    if (
        DIMENSIONAL.search(description)
        and result.category != "dimensional"
    ):
        log.warning("triage_category_smell", said=result.category)
