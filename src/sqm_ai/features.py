...
def build_supplier_month(database_url: str) -> pd.DataFrame:
    """Rebuild sqm.supplier_month and return it as a frame."""
...
def month_folds(df: pd.DataFrame, n_splits: int = 5,
    ...
    gap == label horizon in months so that no training
    label overlaps the test window.
    """
    months = np.sort(df["month"].unique())
    tscv = TimeSeriesSplit(
        n_splits=n_splits, test_size=test_size, gap=gap)
    for tr_m, te_m in tscv.split(months):
        tr = df.index[df["month"].isin(months[tr_m])]
        te = df.index[df["month"].isin(months[te_m])]
        yield tr, te
